...
'    ReDim arr_words(hm_words)
    arr_words(hm_words) = str_the_word
'@@@@@@@ inserting str_the_word into array

'@@@@@@@ inserting inserting picture into array
    
    'ReDim Preserve Picture1(hm_words)
   ' ReDim Picture1(hm_words)
  '  Load Picture1(0)
    Load Picture1(hm_words + 1)
    
    Picture1(hm_words + 1).Visible = True
    my_fileName_for_loading_pic = Mid(sSave, 1, 255 - (hwmc2 - 3))
    Picture1(hm_words + 1).Picture = LoadPicture(my_fileName_for_loading_pic)
    'Picture1(hm_words).Left = Picture1(hm_words).Left + 250
    
    hm_words = hm_words + 1
'@@@@@@@ inserting picture into array
...
End If
        ...
        '@@@@@ loading image into imagebox
         '   For i = 0 To UBound(arr_words)
          '      my_fileName_for_loading_pic = Mid(sSave, 1, 255 - (hwmc2 - 3))
          '      Picture1(i).Picture = LoadPicture(my_fileName_for_loading_pic)
          '  Next
...
str_the_word = arr_words(hm_words - 1) '!!!!!!!!!!!!!!
...
Private Sub cmd_frsh_Click()
    how_much_cmd_try_word = 1
    For i = 0 To cmd_tfz.Count - 1
        cmd_tfz.Item(i).Enabled = True
        
    Next
    
    For i = 0 To cmd_try_word.Count - 1
        cmd_try_word.Item(i).Caption = ""
...
Private Sub cmd_tfz_Click(Index As Integer)
    ...
    If str_the_word = str_does_win Then
        ...
        does_win = True
        Call cmd_frsh_Click
        org_cmd_try_word_Count = cmd_try_word.Count
        For i = (org_cmd_try_word_Count - 1) To 1 Step -1
            Unload cmd_try_word(i)
        Next
        For i = 0 To cmd_tfz.Count - 1
            cmd_tfz.Item(i).Caption = ""
        Next
        Call Form_Load ' for loading the next word
        'Beep
    End If
     '@@@@ check
